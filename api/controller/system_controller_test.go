...
func TestGetHealthSuccess(t *testing.T) {
	...
	tests := []struct {
		expectedCode int
		title        string
		response     domain.HealthRes
	}{
		{
			http.StatusOK, "health-check api test", domain.HealthRes{Message: domain.GetCustomMessage(domain.NoError)},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.title, func(t *testing.T) {
			...
			var responseBody domain.HealthRes
